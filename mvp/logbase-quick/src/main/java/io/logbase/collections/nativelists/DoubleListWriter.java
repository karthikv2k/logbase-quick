 ...
 * User: Kousik
 */
public interface DoubleListWriter extends DoubleConsumer, BatchListWriter<Double> {
  ...
  /**
   * Add #count values from the double supplier to the list.
   *
   * @param supplier
   * @param count
   */
  public default void accept(DoubleSupplier supplier, long count) {
    for (long i = 0; i < count; i++) {
      addPrimitive(supplier.getAsDouble());
    }
  }

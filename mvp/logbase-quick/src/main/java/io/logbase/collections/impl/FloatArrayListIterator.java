 ...
 * User: Kousik
 */
public class FloatArrayListIterator implements FloatListIterator {
  ...
  FloatArrayListIterator(FloatArrayList list, List<ByteBuffer> blocksList, long maxIndex) {
  ...
  @Override
  public int nextPrimitive(float[] buffer, int offset, int count) {
  ...
  public int next(Object buffer, int offset, int count) {
    ...
    checkArgument(buffer instanceof float[], "buffer must be float[], found " + buffer.getClass().getSimpleName());
    return nextPrimitive((float[]) buffer, offset, count);
  }
}
